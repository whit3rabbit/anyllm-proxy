...
pub(crate) async fn send_with_retry<E: RetryableError>(
    ...
    let request_auth = match auth {
        ...
        BackendAuth::AnthropicApiKey(key) => anyllm_client::retry::RequestAuth::Header {
            name: "x-api-key",
            value: key,
        },
        BackendAuth::AnthropicAuthToken(token) => anyllm_client::retry::RequestAuth::Bearer(token),
    };
...
impl BackendClient {
    ...
    pub fn from_backend_config(bc: &BackendConfig) -> Self {
        ...
        let legacy = Config {
            ...
            pxpipe_compress: false,           // not used by BackendClient constructors
