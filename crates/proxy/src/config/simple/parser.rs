...
use super::types::{SimpleConfig, SimpleModelEntry, SimpleParsed, ToolStartupConfig};
use crate::config::litellm::parse_routing_strategy_str;
...
pub fn parse_simple_yaml(yaml: &str) -> SimpleParsed {
    ...
    for entry in &config.models {
        ...
        let api_key = crate::config::helpers::sanitize_api_key(
            &norm
                .api_key
                .clone()
...
fn default_base_url(kind: &BackendKind, entry: &NormalizedEntry) -> String {
    match kind {
        BackendKind::OpenAI => {
            ...
            crate::config::helpers::strip_v1_suffix(&url).to_string()
        }
        BackendKind::Gemini => {
