...
pub(crate) async fn messages(
    ...
    let (body, router_tier) = match router_cfg {
        Some(rc) => {
            ...
            // resolve it through configured model routing before tier-signal
            // classification. resolve_explicit_pick returns None for claude-* alias
            // traffic and unconfigured models, which then fall through to the tiers below.
            match state.resolve_explicit_pick(&body.model) {
                Ok(Some(pick)) => (body, Some(pick)),
                ...
                Ok(None) => {
                    ...
                    let (body, long_context) = if rc.long_context_tier_active() {
                        ...
                        tokio::task::spawn_blocking(move || {
                            let over =
                                super::super::router_signals::is_long_context(&body, threshold);
                            (body, over)
                        })
                        .await
                    ...
                    let signals =
                        super::super::router_signals::anthropic_signals(&body, long_context);
