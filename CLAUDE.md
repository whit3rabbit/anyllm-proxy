...
- **`admin-ui/dist/index.html` is git-tracked and embedded at compile time** via `include_str!` in `admin/routes/mod.rs`. It's a single-file Vite bundle (`viteSingleFile` + `inlineDynamicImports`, ~800KB, all JS/CSS inlined). Deleting it (e.g. during a UI redesign) fails `cargo build` with "couldn't read .../dist/index.html". Fix: `make ui-build` to regenerate, then commit it. Do NOT restore the old file from git (ships stale UI without the new bundle).
