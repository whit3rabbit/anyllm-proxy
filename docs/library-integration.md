...
Research deliverable for Phase 21b. Evaluates how non-Rust consumers can use the `anthropic_openai_translate` crate's translation logic without running the proxy as a separate process.
...
Build: `cbindgen --config cbindgen.toml --crate anthropic_openai_translate --output anthropic_openai_translate.h`
...
lib = ctypes.CDLL("./target/release/libanthropic_openai_translate.so")
...
const lib = koffi.load('./target/release/libanthropic_openai_translate.so');
...
```go
// #cgo LDFLAGS: -L./target/release -lanthropic_openai_translate
// #include "anthropic_openai_translate.h"
import "C"
import "unsafe"
...
pyo3::create_exception!(anthropic_openai_translate, TranslateError, pyo3::exceptions::PyException);
...
#[pymodule]
fn anthropic_openai_translate(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
```python
# anthropic_openai_translate.pyi
...
```python
import json
import anthropic_openai_translate as translator
...
```python
# anthropic_openai_translate/convenience.py (pure Python, wraps native module)
import json
import anthropic_openai_translate._native as _native
